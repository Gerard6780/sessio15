#!/bin/bash
# Script de proves per Lab02 - Fase 2 VLANs
...
LAB_NAME="kea-lab02-vlans"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Lab02 - Fase 2: Proves VLANs + Relay${NC}"
...
check_lab() {
    ...
    if ! docker ps | grep -q "clab-${LAB_NAME}-kea"; then
        ...
        echo "Executa: sudo containerlab deploy -t lab02-vlans/topology.clab.yml"
        exit 1
    fi
